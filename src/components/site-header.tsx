...
export function SiteHeader() {
  return (
    <header className="sticky top-0 z-40 w-full border-b border-[color:var(--line)] bg-white">
      <div className="mx-auto flex max-w-[1440px] items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-3">
          <div className="relative h-11 w-11 overflow-hidden border border-[color:var(--line)] bg-white shadow-[0_8px_18px_rgba(18,13,10,0.14)]">
            <Image
        ...
        <nav className="hidden items-center gap-6 text-sm font-semibold text-black/85 md:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/cart"
            className="inline-flex items-center justify-center border border-[color:var(--berry-dark)] bg-[color:var(--berry)] px-4 py-2 text-xs font-semibold text-white shadow-[0_12px_24px_rgba(18,13,10,0.18)]"
          >
            Cart / Checkout
          </Link>
        </div>
      </div>
      <div className="border-t border-[color:var(--line)] bg-[color:var(--berry-dark)]">
